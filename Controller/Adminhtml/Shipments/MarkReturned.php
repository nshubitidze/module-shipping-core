...
/**
 * Admin row action: mark a shipment as returned to sender.
 *
 * Sets status to {@see ShipmentInterface::STATUS_RETURNED_TO_SENDER} and
 * dispatches `shubo_shipping_returned` for downstream payout handling.
 */
class MarkReturned extends Action
{
    ...
    public function execute(): ResultInterface
    {
        ...
        try {
            $shipment->setStatus(ShipmentInterface::STATUS_RETURNED_TO_SENDER);
            $this->shipmentRepository->save($shipment);

            $this->eventManager->dispatch(
                'shubo_shipping_returned',
                ['shipment' => $shipment],
            );

            $this->messageManager->addSuccessMessage(
                (string)__('Shipment #%1 marked as returned.', $shipmentId),
            );
        } catch (\Throwable $e) {
            $this->logger->error('MarkReturned controller failed.', [
                'shipment_id' => $shipmentId,
                'error' => $e->getMessage(),
            ]);
            $this->messageManager->addErrorMessage(
                (string)__('Could not mark shipment returned: %1', $e->getMessage()),
            );
        }
