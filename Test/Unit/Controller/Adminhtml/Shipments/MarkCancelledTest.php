...
use Shubo\ShippingCore\Controller\Adminhtml\Shipments\MarkCancelled;

class MarkCancelledTest extends TestCase
{
    private MarkCancelled $controller;
    ...
    protected function setUp(): void
    {
        ...
        $this->controller = new MarkCancelled(
            $context,
            $this->repository,
            $this->eventManager,
            $this->logger,
    ...
    public function testExecuteDispatchesCancelledEvent(): void
    {
        $this->request->method('getParam')->with('shipment_id')->willReturn('101');
        $shipment = $this->makeShipment(101, ShipmentInterface::STATUS_PENDING);
        $shipment->expects($this->once())
            ->method('setStatus')
            ->with(ShipmentInterface::STATUS_CANCELLED);
        ...
        $this->eventManager->expects($this->once())
            ->method('dispatch')
            ->with('shubo_shipping_cancelled', ['shipment' => $shipment]);
    ...
    {
        $this->request->method('getParam')->with('shipment_id')->willReturn('66');
    ...
    {
        $this->request->method('getParam')->with('shipment_id')->willReturn('3');
        $shipment = $this->makeShipment(3, ShipmentInterface::STATUS_DELIVERED);
        $this->repository->method('getById')->willReturn($shipment);
    ...
    {
        $this->request->method('getParam')->with('shipment_id')->willReturn('2');
        $shipment = $this->makeShipment(2, ShipmentInterface::STATUS_PENDING);
        $this->repository->method('getById')->willReturn($shipment);
        $this->repository->method('save')->willThrowException(new \RuntimeException('x'));
        $this->logger->expects($this->once())->method('error')
            ->with($this->stringContains('MarkCancelled'), $this->anything());
        $this->messageManager->expects($this->once())
            ->method('addErrorMessage')
            ->with($this->stringContains('Could not cancel shipment'));
        $this->controller->execute();
    }
